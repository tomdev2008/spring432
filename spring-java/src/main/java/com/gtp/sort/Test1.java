...
/**
 * 1.常见的
 * 2.概念 稳定性:(直接插入排序，冒泡排序，归并排序，基数排序)
 * 		假定在待排序的记录序列中，存在多个具有相同的关键字的记录，若经过排序，这些记录的相对次序
 * 		保持不变，即在原序列中，ri=rj，且ri在rj之前，
...
public class Test1 {
	...
	/**
	 * 选择排序是给每个位置选择当前元素最小的，比如给第一个位置选择最小的，在剩余元素里面给第二个
	 * 元素选择第二小的，依次类推，直到第n-1个元素，第n个元素不用选择了，因为只剩下它一个最大的
	 * 元素了
	 */
	@Test
	...
	/**
	 * 归并排序
	 * 所谓“归并”，试讲两个或两个以上的有序文件合并成一个新的有序文件。归并排序是把一个有n个记录
	 * 的无序文件看成是由n个长度为1的有序子文件组成的文件，然后进行两两归并，得到[n/2]个长度为2
	 * 或1的有序文件，再两两归并，如此重复，直至最后形成包含n个记录的有序文件为止
	 */
