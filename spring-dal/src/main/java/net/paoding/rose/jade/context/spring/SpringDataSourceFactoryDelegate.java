...
/**
 * spring之数据源委托工厂类,真实的动作委托给{@link DataSourceFactory}来执行
 */
public class SpringDataSourceFactoryDelegate implements DataSourceFactory {
    ...
    /**
     * 若配置了id="jade.dataSourceFactory"的{@link DataSourceFactory}
     * 则直接使用它，否则让{@link SpringDataSourceFactory}来决定
     */
    @Override
        ...
        if (dataSourceFactory == null) {
            ...
            if (beanFactory != null) {
                if (beanFactory.containsBeanDefinition("jade.dataSourceFactory")) {
                    dataSourceFactory = (DataSourceFactory) beanFactory.getBean("jade.dataSourceFactory", DataSourceFactory.class);
                } else {
