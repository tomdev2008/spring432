...
public class MDao {
	...
	public void up(Person p){
		...
		/*
			...
			equals(Object)
			hashCode()
			isRestrictedTypeKey(String) 
			
			Update.update(String, Object)
			Update.fromDBObject(DBObject, String...)
			Update.set(String, Object) #set
			Update.setOnInsert(String, Object)
			Update.unset(String)#删除
			Update.inc(String, Number)# += 在原有的基础上加上指定的值
			Update.push(String, Object)#追加
			Update.push(String)
			pushAll(String, Object[])
			Update.addToSet(String)#不存在时添加
			Update.addToSet(String, Object)
			Update.pop(String, Position)
			Update.pull(String, Object)#删除
			Update.pullAll(String, Object[])
			Update.rename(String, String)
			Update.getUpdateObject()
			Update.addFieldOperation(String, String, Object)
			Update.addMultiFieldOperation(String, String, Object)
			Update.modifies(String)
			Update.isKeyword(String)
			
			Criteria()
			Criteria(String)
			Criteria(List<Criteria>, String)
			where(String)#创建查询条件  类似于 where string=?
			and(String)# AND
			is(Object)#=
			lastOperatorWasNot()
			ne(Object)#!=
			...
			registerCriteriaChainElement(Criteria)
			getKey()
			getCriteriaObject()
			getSingleCriteriaObject()
			createCriteriaList(Criteria[])
			setValue(DBObject, String, Object)
			equals(Object)
			simpleCriteriaEquals(Criteria, Criteria)
