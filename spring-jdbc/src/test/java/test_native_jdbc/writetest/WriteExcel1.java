package test_native_jdbc.writetest;
...
public class WriteExcel1 {
	...
	public void test() {
		try {
			...
			/************************ 最基本的操作 ***************************/
			/*
			 * 创建对象，层次关系是： workbook-->sheet-->row-->cell 工作簿 表 行 列
			 */
			// 创建工作簿（内存中）
			...
			row.createCell(4).setCellValue(new HSSFRichTextString("dddddddddddddddddddddddddddddddddddd"));// 长字符串
			...
			style = wb.createCellStyle();// 创建样式
			style.setWrapText(true);// 自动换行 适合大文本这样的数据列
			...
			style = wb.createCellStyle();
			style.setBorderTop(HSSFCellStyle.BORDER_DOUBLE);// 上边框，双实线
			...
			row.createCell(0).setCellValue("合并单元格:2行4列");

			// 合并4-5行 0-6列单元格
			sheet.addMergedRegion(new CellRangeAddress(4, 5, 0, 6));
			...
			wb.write(new FileOutputStream("E:/2017/w1.xls"));
			wb.close();
		} catch (Exception e) {
			e.printStackTrace();
		}
